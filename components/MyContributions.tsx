            ...
            <Item>
              <div className='flex flex-col gap-2'>
                <div className='flex flex-row gap-4'>
                  <a href={contribution.repository.owner.profileUrl} target='_blank' rel='noreferrer nofollow'>
                    <img
                ...
                <div className='mt-2'>
                  <Link href={contribution.htmlUrl}>
                    Ver PR
                  </Link>
                </div>
              </div>
...
function ContributionsStats({ total, uniqueRepository, isLoading = false }: Readonly<ContributionsStatsProps>) {
  return (
      ...
      <div className={clsx('sticky top-24 z-10 w-64', 'xl:w-[272px]', 'fm:relative fm:top-0')}>
        <div className={clsx(
          ...
          <div className='mb-4 pb-3 border-b border-divider-light dark:border-divider-dark'>
            <div className='font-bold text-lg text-slate-800 dark:text-slate-200'>Mis contribuciones</div>
          </div>
          <div className='space-y-3'>
            <div className='flex flex-row justify-between items-center py-2'>
              <div className='font-medium text-sm text-slate-600 dark:text-slate-400'>Total contribuciones</div>
              {isLoading ? (
            ...
            <div className='flex flex-row justify-between items-center py-2'>
              <div className='font-medium text-sm text-slate-600 dark:text-slate-400'>Repositorios únicos</div>
              {isLoading ? (
                <Skeleton w={40} h={20} isLoading className='my-1'/>
              ) : (
                <div className='font-bold text-lg text-green-600 dark:text-green-400'>{uniqueRepository}</div>
              )}
            </div>
          </div>
        </div>
